 ...
 * @LastEditTime: 2025-09-07 17:49:09
 * @FilePath: /Server-framework/source/include/sylar/log/log.h
 * @Description:
 *
 ...
 *
 */
#define SYLARY_LOG_FMT_DEBUG(logger, fmt, ...) \
  SYLARY_LOG_FMT_LEVEL(logger, sylar::LogLevel::DEBUG, fmt, __VA_ARGS__)

/**
 * @brief 使用格式化方式将日志级别info的日志写入到logger
 *
 */
#define SYLARY_LOG_FMT_INFO(logger, fmt, ...) \
  SYLARY_LOG_FMT_LEVEL(logger, sylar::LogLevel::INFO, fmt, __VA_ARGS__)

/**
 * @brief 使用格式化方式将日志级别warn的日志写入到logger
 *
 */
#define SYLARY_LOG_FMT_WARN(logger, fmt, ...) \
  SYLARY_LOG_FMT_LEVEL(logger, sylar::LogLevel::WARN, fmt, __VA_ARGS__)

/**
 * @brief 使用格式化方式将日志级别error的日志写入到logger
 *
 */
#define SYLARY_LOG_FMT_ERROR(logger, fmt, ...) \
  SYLARY_LOG_FMT_LEVEL(logger, sylar::LogLevel::ERROR, fmt, __VA_ARGS__)

/**
 * @brief 使用格式化方式将日志级别fatal的日志写入到logger
 *
 */
#define SYLARY_LOG_FMT_FATAL(logger, fmt, ...) \
  SYLARY_LOG_FMT_LEVEL(logger, sylar::LogLevel::FATAL, fmt, __VA_ARGS__)
...
#define SYLAR__SYSTEM__LOG(name) \
  sylar::Logger::ptr name = SYLAR_LOG_NAME("system")
...
class LogLevel {
 public:
   ...
   * @return const char*
   *
   * @details { ERROR：没有对象无法调用成员函数 }
   * 成员函数需要对象调用，所以声明为static
