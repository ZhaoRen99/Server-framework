 ...
 * @LastEditTime: 2025-09-06 16:04:29
 * @FilePath: /Server-framework/source/include/sylar/http/http.h
 * @Description: 
 * 
...
class HttpRequest {
 public:
  ...
  template <class T>
  bool cheakGetHeaderAs(const std::string& key, T& val, const T& def = T()) {
  ...
  template <class T>
  bool cheakGetParamAs(const std::string& key, T& val, const T& def = T()) {
  ...
  template <class T>
  bool cheakGetCookieAs(const std::string& key, T& val, const T& def = T()) {
...
class HttpResponse {
 public:
  ...
  void setStatue(HttpStatus v) { m_status = v; }
  void setVersion(uint8_t v) { m_version = v; }
  void setBody(const std::string& v) { m_body = v; }
