...
left); if (rt <= 0) { close(); return nullptr;
                    }
                    body.append(data, rt);
                    left -= rt;
                }
...
HttpResult::ptr HttpConnection::DoGet(
    const std::string& uri, uint64_t timeout_ms,
    const std::map<std::string, std::string>& headers,
    const std::string& body) {
  Uri::ptr _uri = Uri::Create(uri);
  if (!_uri) {
    return std::make_shared<HttpResult>((int)HttpResult::Error::INVAILD_URL,
                                        nullptr, "invalid uri: " + uri);
  }
  return DoGet(_uri, timeout_ms, headers, body);
}

HttpResult::ptr HttpConnection::DoGet(
    Uri::ptr url, uint64_t timeout_ms,
    const std::map<std::string, std::string>& headers,
    const std::string& body) {
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req, Uri::ptr uri,
  ...
  if (!rsp) {
    return std::make_shared<HttpResult>(
        ...
        "recv response fail, addr: " + addr->toString() +
...
HttpResult::ptr HttpConnectionPool::doGet(
    ...
    const std::map<std::string, std::string>& headers,
    const std::string& body) {
  return doRequest(http::HttpMethod::GET, url, timeout_ms, headers, body);
}
...
HttpResult::ptr HttpConnectionPool::doPost(
    ...
    const std::map<std::string, std::string>& headers,
    const std::string& body) {
  return doRequest(http::HttpMethod::POST, url, timeout_ms, headers, body);
}
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req,
  ...
  if (!rsp) {
    return std::make_shared<HttpResult>(
        (int)HttpResult::Error::TIMEOUT, nullptr,
        "recv response fail, addr: " + sock->getRemoteAddress()->toString() +
            ", timeout_ms: " + std::to_string(timeout_ms));
  }
  return std::make_shared<HttpResult>((int)HttpResult::Error::OK, rsp, "ok");
}
