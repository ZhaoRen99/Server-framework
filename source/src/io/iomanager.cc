...
IOManager::IOManager(size_t threads, bool use_caller, const std::string& name)
  ...
  // F_SETFL: 获取/设置文件状态标志
  // O_NONBLOCK:
  // 使I/O变成非阻塞模式，在读取不到数据或是写入缓冲区已满会马上return，而不会阻塞等待。
...
void IOManager::idle() {
  ...
  while (true) {
    ...
    if (stopping(next_timeout)) {
      SYLAR_LOG_INFO(g_logger) << sylar::Thread::GetName() << " is done";
      break;
    }
    ...
    do {
      ...
      if (next_timeout != ~0ull) {
        next_timeout =
            (int)next_timeout > MAX_TIMEOUT ? MAX_TIMEOUT : (int)next_timeout;
      } else {
