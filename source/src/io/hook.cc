...
// dlsym -
// 从一个动态链接库或者可执行文件中获取到符号地址。成功返回跟name关联的地址
...
retry:
  ...
  SYLAR_LOG_DEBUG(sylar::g_logger)
      << "do_io <" << hook_fun_name << ">" << " n=" << n << " errno=" << errno;
  // 中断则重试
...
unsigned int sleep(unsigned int seconds) {
   ...
   *(void(sylar::Scheduler::*)(sylar::Fiber::ptr, int thread))
   是一个函数指针类型， *它定义了一个指向 sylar::Scheduler 类中一个参数为
   sylar::Fiber::ptr 和 int 类型的成员函数的指针类型。 *具体来说，它的含义如下：
   ...
   * 在这里，第二个参数使用了函数指针类型
   (void(sylar::Scheduler::*)(sylar::Fiber::ptr, int thread))
   * ，表示要绑定的函数类型是 sylar::Scheduler 类中一个参数为 sylar::Fiber::ptr
   和 int 类型的成员函数
   * ，这样 std::bind 就可以根据这个函数类型来实例化出一个特定的函数对象，并将
   fiber 和 -1 作为参数传递给它。
   */
  iom->addTimer(
      seconds * 1000,
      std::bind((void (sylar::Scheduler::*)(
