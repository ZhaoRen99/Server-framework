...
static int32_t DecodeZigzag(const uint32_t& v) { return (v >> 1) ^ -(v & 1); }

static int64_t DecodeZigzag(const uint64_t& v) { return (v >> 1) ^ -(v & 1); }
...
bool ByteArray::writeToFile(const std::string& name) const {
  ...
  while (read_size > 0) {
    ...
    int64_t len = (read_size + diff) > (int64_t)m_baseSize ? (m_baseSize - diff)
                                                           : read_size;
    // SYLAR_LOG_DEBUG(g_logger) << "len = " << len << "LEN = "<< LEN;
...
void ByteArray::addCapacity(size_t size) {
  ...
  size = size - old_cap;
  // size_t count = (size / m_baseSize) + (((size % m_baseSize) > old_cap) ? 1 :
  // 0);
...
uint64_t ByteArray::getReadBuffers(std::vector<iovec>& buffers, uint64_t len,
  ...
  // XXX: 修改cur
  // size_t count = npos ? (position / m_baseSize + 1) : (position /
  // m_baseSize); Node* cur = m_root; while (count > 1) {
  //     cur = cur->next;
  //     --count;
  // }
