...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
  ...
  while (next) {
    ...
    // ((sockaddr_in*)next->ai_addr)->sin_addr.s_addr; SYLAR_LOG_DEBUG(g_logger)
    // << next->ai_canonname;
    next = next->ai_next;
  }
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
  ...
  if (error) {
    SYLAR_LOG_ERROR(g_logger)
        ...
        << ") error = " << error << ", gai_strerror = " << gai_strerror(error);
    return nullptr;
  }
...
IPv4Address::IPv4Address(const sockaddr_in& sockaddr) { m_addr = sockaddr; }
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
  ...
  int result =
      inet_pton(AF_INET6, address, &rt->m_addr.sin6_addr.__in6_u.__u6_addr8);
  if (result <= 0) {
    SYLAR_LOG_ERROR(g_logger)
        << "IPv4Address::Create(" << address << ", " << port
        << ") rt = " << result << " errno = " << errno
...
IPv6Address::IPv6Address(const sockaddr_in6& sockaddr) { m_addr = sockaddr; }
...
void UinxAddress::setAddrlen(uint32_t v) { m_length = v; }

const sockaddr* UinxAddress::getAddr() const { return (sockaddr*)&m_addr; }

socklen_t UinxAddress::getAddrLen() const { return m_length; }
...
UnknowAddress::UnknowAddress(const sockaddr& address) { m_addr = address; }
