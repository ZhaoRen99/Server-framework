 ...
 * @LastEditTime: 2025-09-08 13:39:01
...
std::string g_log_path = TOP_DIR_PATH + std::string("/conf/log.yml");

std::string g_test_path = TOP_DIR_PATH + std::string("/conf/test.yml");
...
void test_yaml() {
  YAML::Node root = YAML::LoadFile(g_log_path);
  print_yaml(root, 0);
...
#define XX_M(g_var, name, prefix)                                          \
  ...
  XX(g_string_value_config, name, before);
  ...
  YAML::Node root = YAML::LoadFile(g_test_path);
  ...
  XX(g_string_value_config, name, after);
